...
class ScannerAgent(Agent):
    ...
    def make_user_prompt(self, scraped_jobs: List[ScrapedJobDescription]) -> str:
        """
        Create a user prompt for OpenAI based on scraped job descriptions.
        """
        user_prompt = self.USER_PROMPT_PREFIX
        user_prompt += '\n\n'.join([job.describe() for job in scraped_jobs])
        user_prompt += self.USER_PROMPT_SUFFIX
        return user_prompt

    def scan(self, memory: List[ScrapedJobDescription] = []) -> Optional[JobSelection]:
        """
        Call OpenAI to process job descriptions and return the most promising ones.
        """
        scraped_jobs = self.fetch_jobs(memory)
        if scraped_jobs:
            user_prompt = self.make_user_prompt(scraped_jobs)
            ...
            self.log(f"Job Scanner Agent received {len(parsed_result.jobs)} selected jobs from OpenAI")
